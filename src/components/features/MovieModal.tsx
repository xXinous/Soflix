      ...
      <div className="bg-gray-900 rounded-lg max-w-xs sm:max-w-lg md:max-w-2xl lg:max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
        ...
        <div className="relative h-48 sm:h-56 md:h-64 lg:h-80">
          ...
          <div className="absolute bottom-3 sm:bottom-4 md:bottom-6 left-3 sm:left-4 md:left-6 right-3 sm:right-4 md:right-6">
            <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl mb-2 sm:mb-3 md:mb-4 line-clamp-2">{movie.title}</h2>
            <div className="flex items-center space-x-2 sm:space-x-3 overflow-x-auto pb-2">
              <button className="bg-white text-black px-3 sm:px-4 md:px-6 py-1.5 sm:py-2 rounded-md flex items-center space-x-1 sm:space-x-2 hover:bg-gray-200 transition-colors text-sm sm:text-base whitespace-nowrap">
                <Play className="w-4 h-4 sm:w-5 sm:h-5" />
                <span>Reproduzir</span>
              </button>
              <button className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 bg-gray-600/50 rounded-full flex items-center justify-center hover:bg-gray-600 transition-colors flex-shrink-0">
              ...
              <button className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 bg-pink-600 rounded-full flex items-center justify-center hover:bg-pink-700 transition-colors flex-shrink-0">
        ...
        <div className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-5 md:space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4">
            <span className="text-green-400 font-semibold text-sm sm:text-base">{movie.rating} Compatibilidade</span>
            <span className="text-gray-400 text-sm sm:text-base">{movie.year}</span>
            <span className="border border-gray-500 px-2 py-1 text-xs rounded">{movie.genre}</span>
          ...
          <div className="grid sm:grid-cols-2 gap-4 sm:gap-6 pt-3 sm:pt-4 border-t border-gray-700">
            <div className="space-y-2 sm:space-y-3">
              <div className="text-sm sm:text-base">
                ...
                <span>Romance, Drama, Comédia Romântica</span>
              </div>
              <div className="text-sm sm:text-base">
                <span className="text-gray-400">Este filme é: </span>
                <span>Romântico, Emocionante, Tocante</span>
              </div>
            </div>
            <div className="space-y-2 sm:space-y-3">
              <div className="text-sm sm:text-base">
          ...
          <div className="bg-gradient-to-r from-pink-600/20 to-purple-600/20 p-3 sm:p-4 md:p-6 rounded-lg border-l-4 border-pink-500">
            <p className="text-sm sm:text-base md:text-lg italic text-center">
              "Com grandes poderes vêm grandes responsabilidades... e com grande amor vem grande felicidade!"
            </p>
          </div>

          {/* Special Photos for Spider Couple */}
          {movie.specialPhotos && movie.specialPhotos.length > 0 && (
            <div className="space-y-3 sm:space-y-4">
              <h3 className="text-base sm:text-lg flex items-center space-x-2">
                <Heart className="w-4 h-4 sm:w-5 sm:h-5 text-pink-500 fill-current" />
                <span className="text-sm sm:text-base">Momentos Especiais do Casal-Aranha</span>
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                {movie.specialPhotos.map((photo, index) => {
                  const captions = [
          ...
          {/* Default Similar Movies for other films */}
          {(!movie.specialPhotos || movie.specialPhotos.length === 0) && (
            <div className="space-y-3 sm:space-y-4">
